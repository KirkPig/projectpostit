...
public class BLSelection extends VBox {
	...
	public BLSelection() {
		...
		newButton.setOnMouseClicked((MouseEvent e) -> {
			...
			Scene blnewScene = new Scene(new BLNewUI(newStage));
			newStage.setScene(blnewScene);
			newStage.show();
		});
		...
		// =============================================================================
		table = new TableView<ProductLoan>();
		TableColumn<ProductLoan, String> code = new TableColumn<ProductLoan, String>("ID");
		code.setMinWidth(60);
		code.setCellValueFactory(new PropertyValueFactory<>("id"));
		TableColumn<ProductLoan, String> date = new TableColumn<ProductLoan, String>("date");
		date.setMinWidth(100);
		date.setCellValueFactory(new PropertyValueFactory<>("date"));
		TableColumn<ProductLoan, String> customerName = new TableColumn<ProductLoan, String>("Customer Name.");
		customerName.setMinWidth(200);
		customerName.setCellValueFactory(new PropertyValueFactory<>("customerName"));
		TableColumn<ProductLoan, String> totalAmount = new TableColumn<ProductLoan, String>("Total Amount");
		totalAmount.setCellValueFactory(new PropertyValueFactory<>("valueAfterTaxForTable"));
		totalAmount.setMinWidth(120);
		TableColumn<ProductLoan, String> creator = new TableColumn<ProductLoan, String>("Created by");
...
//=========================
		...
		TableColumn<ProductLoan, String> dateCol = new TableColumn<ProductLoan, String>("date");
		dateCol.setCellValueFactory(new PropertyValueFactory<>("date"));
		TableColumn<ProductLoan, String> descriptionCol = new TableColumn<ProductLoan, String>("Description");
		descriptionCol.setMinWidth(600);
		descriptionCol.setCellValueFactory(new PropertyValueFactory<>("productName"));
		TableColumn<ProductLoan, String> quantityCol = new TableColumn<ProductLoan, String>("Quantity");
		quantityCol.setMinWidth(100);
		quantityCol.setCellValueFactory(new PropertyValueFactory<>("productQuantity"));
		TableColumn<ProductLoan, String> unitCol = new TableColumn<ProductLoan, String>("Unit");
	...
	public static void updateBL(String search) {
		try {
			...
			String sql = "select * from productloan;";
			...
			while (rs.next()) {
				...
				String blProductString = rs.getString("product");
				
				blProductString.replaceAll("\n", " ");
				ArrayList<Item> itemList = gson.fromJson(blProductString, token.getType());
				...
				if (genre.getValue() != null && !search.isEmpty()) {
					...
					case "Amount":
						try {
							Double searchNum = Double.parseDouble(search);
							addToTable = (searchNum == productloan.getValueAfterTax());
						} catch (NumberFormatException nfe) {
							break;
						}
						break;
				...
				if (addToTable) {
					...
					table.getItems().add(productloan);
					table2.getItems().add(productloan);
	...
	public static void delete(ProductLoan productloan) {
		...
		try {
			...
			String sql = "DELETE from productloan WHERE id ='" + productloan.getId() + "'";
	...
	public SortedSet<String> getCodeTree() {
		try {
			...
			Connection conn = DatabaseConnection.getConnection();
			Statement stmt = conn.createStatement();
			String sql = "select id from productloan";
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
	...
	public SortedSet<String> getCreatorTree() {
		try {
			...
			Connection conn = DatabaseConnection.getConnection();
			Statement stmt = conn.createStatement();
			String sql = "select user from productloan";
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
	...
	public SortedSet<String> getAmountTree() {
		try {
			...
			Connection conn = DatabaseConnection.getConnection();
			Statement stmt = conn.createStatement();
			String sql = "select valueaftertax from productloan";
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
