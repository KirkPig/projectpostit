...
public class PONewUI extends VBox {
	...
	public PONewUI(Stage yourOwnStage) {
		createNew = true;
		...
		po = new POBox(300, 120);
		cusBox = new CustomerBox(300, 250);
		left.getChildren().addAll(genBox, po);
		left.setSpacing(4);
	...
	public PONewUI(Stage yourOwnStage, Order order) {
		this(yourOwnStage);
		createNew = false;
		id = order.getId();
		genBox.setGenBox(order.getId(), order.getDate());
		cusBox.setSelectedCustomer(order.getCustomer());
		po.setPaymentText(order.getPaymentTerm());
		productTable.getItems().addAll(order.getItemList());
		calculateTax();
	}
	...
	public String generateId(String date) {
		try {
			...
			while (true) {
				gid = "PO" + date.substring(8) + "0" + date.substring(3, 5) + String.format("%03d", k);
				String str = "select * from productorder where id like '" + gid + "'";
