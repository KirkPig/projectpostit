...
public class DENewUI extends VBox {
	...
	public DENewUI(Stage yourOwnStage) {
		...
		de = new DEBox(300, 120);
		cusBox = new CustomerBox(300, 250);
		left.getChildren().addAll(genBox, de);
		left.setSpacing(4);
	...
	public String generateId(String date) {
		try {
			...
			while (true) {
				gid = "DE" + date.substring(8) + "0" + date.substring(3, 5) + String.format("%03d", k);
				String str = "select * from delivery where id like '" + gid + "'";
